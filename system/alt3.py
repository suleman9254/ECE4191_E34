...
class Robot(object):
    ...
    def reverse(self, dist):
        goal = self.transform(-dist, th=0); self.drive(goal)
    ...
    def cameraDrive(self, size, tgtProx, thTol, alpha, beta, detector, xBound=inf, yBound=inf, lastAlpha=1):
        ...
        goal = self.transform(dist, th, alpha=lastAlpha)
        _ = self.drive(goal)
    ...
    def deliver(self, camProx, sonicProx, sensorTimeout, thTol, alpha, beta):
        ...
        approached = self.cameraDrive(self.boxHeight, camProx, thTol, alpha, beta, self.detector.find_box, lastAlpha=0.7)
        
        while not approached and not timeout:
            self.explore(); timeout = time() - start_time < sensorTimeout
            approached = self.cameraDrive(self.boxHeight, camProx, thTol, alpha, beta, self.detector.find_box, lastAlpha=0.7)

        self.rail.set_position(1)

        if approached: self.sensorDrive(self.ultrasonic, sonicProx, alpha=0.4, maxTime=sensorTimeout)

        self.claw.release()
        self.reverse(dist=minProx)
        self.rail.set_position(0)
        self.turn(2 * pi / 4)
    ...
    def start(self, onTime, tgtBallProx, tgtBoxProx, thBallTol, thBoxTol, grabDist, deliverDist, alpha, beta, collectSensorTimeout=5, deliverSensorTimeout=30):
        ...
        while time() - startTime < onTime:
            ...
            try:
                ...
                self.claw.release(); self.rail.set_position(0); break
